...
if __name__ == '__main__':
    ...
    slides = list(map(lambda x: int(x[:-4]), filter(lambda x: x.endswith('.pdf'), os.listdir(os.path.join(source, 'translated')))))
    print('slides:', slides)
    ...
    for slide_id in slides:
        ...
        output_file = os.path.join(target, '%02d.txt' % slide_id)
